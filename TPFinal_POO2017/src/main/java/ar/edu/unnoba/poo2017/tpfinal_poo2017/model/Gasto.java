...
@NamedQueries({
    ...
    ,
         @NamedQuery(name = "gastos.cantidad", query = "SELECT COUNT(g.id) FROM Gasto g")
    ,
      @NamedQuery(name = "gastos.cantidad.empresa", query = "SELECT COUNT(g.id) FROM Gasto g WHERE g.empresa = :empresa")
})
